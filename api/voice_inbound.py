...
def process_speech_update_call(
    call_sid: str, inbound_phone: str, audio_url: str
) -> None:
    """
    ...
    try:
        ...
        response.say(
            f"I'm sorry, sir. There was an error. {str(e)}", voice=RESPONSE_VOICE
        )
...
async def incoming_call(request: Request):
    ...
    logger.info(
        f"inboundcall-{form['CallSid']}: INFO: Handler picked up incoming call."
    )
...
async def process_speech(background_tasks: BackgroundTasks, request: Request):
    ...
    background_tasks.add_task(
        process_speech_update_call, call_sid, phone_number, audio_url
    )
